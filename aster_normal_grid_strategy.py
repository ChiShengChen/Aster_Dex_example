...
def calculate_grid_levels(upper_price, lower_price, num_grids):
    ...
    return sorted(list(set(formatted_levels)), reverse=True) # Return unique, sorted levels (highest first)
...
if __name__ == "__main__":
    ...
    # --- Initial Setup ---
    print("--- Grid Strategy Initializing ---")
        ...
        try:
            ...
            for level_price in grid_levels:
                 ...
                 if level_price < current_price:
                     ...
                     if level_price not in open_order_prices['BUY']:
                         print(f"Missing BUY order at {level_price}. Placing...")
                         place_result = place_limit_order(TARGET_SYMBOL, 'BUY', decimal.Decimal(str(ORDER_QTY_PER_GRID)), level_price)
                         if place_result and 'orderId' in place_result:
                              print(f"  Successfully placed BUY order {place_result['orderId']} at {level_price}")
                              placed_orders_this_cycle += 1
                              time.sleep(0.2) # Small delay between placements
                         else:
                              print(f"  Failed to place BUY order at {level_price}. Response: {place_result}")
                              # Consider adding retry logic or error tracking here
                 
                 # --- Sell Side Logic ---
                 elif level_price > current_price:
                      # Should have a SELL order, unless it's already open
                      if level_price not in open_order_prices['SELL']:
                           print(f"Missing SELL order at {level_price}. Placing...")
                           place_result = place_limit_order(TARGET_SYMBOL, 'SELL', decimal.Decimal(str(ORDER_QTY_PER_GRID)), level_price)
                           if place_result and 'orderId' in place_result:
                                print(f"  Successfully placed SELL order {place_result['orderId']} at {level_price}")
                                placed_orders_this_cycle += 1
                                time.sleep(0.2) # Small delay between placements
                           else:
                                print(f"  Failed to place SELL order at {level_price}. Response: {place_result}")
                                # Consider adding retry logic or error tracking here
